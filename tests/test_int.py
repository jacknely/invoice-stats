...
def test_input_1(mocker, capsys):
    ...
    captured = capsys.readouterr()
    captured = captured.out.split("\n")
    assert captured[-2] == "[70.0, 100.0, 20.54]"
...
def test_input_5(mocker, capsys):
    ...
    mocker.patch('builtins.input', side_effect=["5", "7"])
    run.main()
    captured = capsys.readouterr()
    captured = captured.out.split("\n")
    assert captured[-2] == "The mean is: 63.51"


def test_input_3(mocker, capsys):
    """ method for testing option 3 from start menu """
    mocker.patch('builtins.input', side_effect=["3", "7"])
    run.main()
    captured = capsys.readouterr()
    print(captured.out)
    captured = captured.out.split("\n")
